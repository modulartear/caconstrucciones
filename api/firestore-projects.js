...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      const data = { id: docId, title, location, status, surface, year, description, cover, gallery: gallery || [], progress: progress || 0 };
      await db.collection('projects').doc(docId).set(data, { merge: true });
