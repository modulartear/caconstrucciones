...
async function prepareInputs(template) {
  ...
  const rawMaskBuffer = await sharp(maskPath)
    ...
    .png()
    .toBuffer();
  const maskBuffer = await convertSurfaceMaskForOpenAI(rawMaskBuffer, template.surfaceMaskMode || 'alpha-cutout');

  return { imageBuffer, maskBuffer };
}

async function convertSurfaceMaskForOpenAI(maskBuffer, maskMode) {
